...
procedure TFormMain.BtnGerarBackupClick(Sender: TObject);
var dirCharacterdata, dirConf, dirMinimap, dirScreenshots : String;
begin
  if CkbCopiarTestServer.Checked = True then
    begin
      dirCharacterdata := GetEnv('USERPROFILE')+'\AppData\Local\Tibia\packages\TibiaExternal\characterdata';
      dirConf          := GetEnv('USERPROFILE')+'\AppData\Local\Tibia\packages\TibiaExternal\conf';
      dirMinimap       := GetEnv('USERPROFILE')+'\AppData\Local\Tibia\packages\TibiaExternal\minimap';
      dirScreenshots   := GetEnv('USERPROFILE')+'\AppData\Local\Tibia\packages\TibiaExternal\screenshots';
    end else
    begin
...
begin
  if CkbCopiarTestServer.Checked = True then
    BtnGerarBackup.Caption := 'Copiar Test Server'
    else
    BtnGerarBackup.Caption := 'Gerar Backup';
end;
